...
`src/mcp/server.rs` 当前测试前已有 546 行生产代码，目标改动触发“超过 500 行先拆再改”规则。实现前把 `tool_call_result_to_mcp`、`invoke_result_to_mcp` 与 content-defense 前缀处理移动到私有 `mcp/result.rs`；`server.rs` 只导入转换函数并保留协议 handler、认证、catalog 解析与 executor 装配。该拆分不改变 MCP 返回形状。
...
- MCP 忽略格式 override 是行为变更；实现时必须同步更正 [Response Rendering](response-rendering.md)、[Compatibility Policy](compatibility-policy.md) 与 [Documentation Log](log.md)。
