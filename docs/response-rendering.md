...
自动化覆盖见 `src/render.rs`（markdown 规则单测）、`src/proxy/executor.rs`（yaml/markdown/透传/error 跳过的 invoke 集成测试）、`src/http/mod.rs`（`?format=` / `Accept` / 非法值 / key 级配置的 HTTP 测试）。以下为手动冒烟流程，用于对真实上游确认约定行为。

## 流程

两个上游各验证一类边界：**远程 MCP（Exa）**验格式协商 + 非 JSON 透传，**HTTP JSON 上游**验渲染真实生效。
...
3. 对同一工具依次请求 `?format=`（缺省 / `yaml` / `markdown` / 非法值）与 `Accept:` 协商，比对响应头 `x-asterlane-format`、`Content-Type` 与 body。
...
Exa MCP 的 `web_search_exa` / `web_fetch_exa` 返回的 `content[].Text` 是**预格式化纯文本**（`Title:… URL:… Highlights:…`），不是 JSON。因此对 Exa 请求 `yaml` / `markdown` 命中"非 JSON 透传"分支——这是符合转换边界的**正确行为**，不是缺陷：Exa 本就返回人类可读文本，无需再转。格式协商链（含非法值 fail-fast）对所有上游一致生效。
